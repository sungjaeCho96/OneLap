...
export async function fetchQualifyingSessionList(): Promise<QualifyingSessionOption[]> {
  try {
    const res = await fetch(
      'https://api.openf1.org/v1/sessions?session_name=Qualifying&year=2026',
      { next: { revalidate: 3600 } },
    )
    if (!res.ok) throw new Error('OpenF1 qualifying sessions error')

    const sessions: OpenF1QualiSession[] = await res.json()
